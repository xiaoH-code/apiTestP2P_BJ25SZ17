...
import random
import app
from utils import assert_utils, DButils, get_img_verify_code_data, get_register_data, read_param_data
...
class TestLogin(unittest.TestCase):
    ...
    # 测试获取图片验证码脚本
    #@parameterized.expand(get_img_verify_code_data)  —— 分别读取
    @parameterized.expand(read_param_data("img_verify_code.json","get_img_verify_code","type,status_code"))
    def test01_get_img_code(self,type,status_code):
        # 准备测试数据
        if type == "float":
            r_data = random.random()
        elif type == "int":
            r_data = random.randint(10000000,99999999)
        elif type == "kong":
            r_data = ""
        elif type == "char":
            r_data = "".join(random.sample("abcdefghiljklmn",8))
    ...
    # 测试注册用例
    # @parameterized.expand(get_register_data) —— 单独读取
    @parameterized.expand(read_param_data("register.json","test_register","phone,pwd,imgCode,phoneCode,dyServer,invitePhone,status_code,status,description"))
    def test10_register(self,phone,pwd,imgCode,phoneCode,dyServer,invitePhone,status_code,status,desc):
        #1、获取图片验证码
        # 准备测试数据
        r_data = random.random()
        # 调用封装API接口，传入测试数据，接收API接口返回的响应
        ...
        # 准备测试数据
        # 调用封装API接口，传入测试数据，接收API接口放回的响应
        response = self.login_api.get_sms_verify_code(self.session, phone, app.imgCode)  # 使用app定义全局参数（变量）
        logging.info('response = {}'.format(response.json()))
        # 对返回的响应内容进行断言
        assert_utils(self,response,200,200,"短信发送成功")
        # 3、注册
        # 准备测试数据
        # 调用接口的方法
        response = self.login_api.register(self.session,phone,pwd,imgCode,phoneCode,dyServer,invitePhone)
        logging.info('response = {}'.format(response.json()))
        # 对返回的响应内容进行断言
